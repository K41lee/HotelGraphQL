    ...
    public CatalogDTO getCatalog(String hotelCode) {
        try {
            ...
            if (catalog.hasHotel()) {
                ...
                // Ajouter les villes (si disponibles dans la description)
                if (!catalog.getHotel().getDescription().isEmpty()) {
                    dto.setCities(Arrays.asList(catalog.getHotel().getDescription()));
                }
    ...
    private OfferDTO convertOfferToDTO(Offer protoOffer, String hotelCode) {
        try {
            ...
            if (protoOffer.hasRoom()) {
                ...
                int imageCount = protoOffer.getRoom().getImagesCount();
                log.info("[gRPC-CLIENT] Room {} has {} images", roomNumber, imageCount);
                if (imageCount > 0) {
                    String imageUrl = protoOffer.getRoom().getImages(0).getUrl();
                    roomDto.setImageUrl(imageUrl);
                    log.info("[gRPC-CLIENT] Image URL set for room {}: {}", roomNumber,
                        imageUrl.substring(0, Math.min(50, imageUrl.length())) + "...");
                } else {
                    log.warn("[gRPC-CLIENT] No images for room {}", roomNumber);
                }
